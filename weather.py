...
def task3_compute_statistics():
    ...
    daily_stats = df.resample('D').agg({'Temperature': ['mean', 'min', 'max', 'std'], 'Rainfall': ['mean', 'min', 'max', 'std'],'Humidity': ['mean', 'min', 'max', 'std']})
    
    monthly_stats = df.resample('M').agg({'Temperature': ['mean', 'min', 'max', 'std'], 'Rainfall': ['mean', 'min', 'max', 'std'],'Humidity': ['mean', 'min', 'max', 'std']})
    
    yearly_stats = df.resample('Y').agg({'Temperature': ['mean', 'min', 'max', 'std'],'Rainfall': ['mean', 'min', 'max', 'std'],'Humidity': ['mean', 'min', 'max', 'std']})
...
def task5_group_and_aggregate():
    ...
    monthly_group = df.groupby('Month').agg({'Temperature': ['mean', 'min', 'max'],'Rainfall': 'sum','Humidity': 'mean'})
    ...
    seasonal_group = df.groupby('Season').agg({'Temperature': ['mean', 'min', 'max'],'Rainfall': 'sum','Humidity': 'mean'})
    print("\nSeasonal Aggregates:")
